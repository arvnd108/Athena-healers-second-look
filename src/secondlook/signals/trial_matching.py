...
Named `trial_matching` rather than `trials` because `signals/trials.py` is
already taken by Subsystem E's trials *signal generator*, which queries the
graph for Trial nodes along an OncoSphere dimension. This module answers a
different question -- whether one case satisfies one trial's criteria -- and
