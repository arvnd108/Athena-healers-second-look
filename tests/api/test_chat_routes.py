...
def test_phase6_retrieval_grounding_in_turn():
    ...
    create_res = client.post(
        "/api/chat/sessions",
        json={"model_id": "mock-outline", "attachment_ids": ["variant-normalizer", "citation-guard"]},
    )
    ...
    assert any("Larotrectinib" in s.get("title", "") or "Larotrectinib" in s.get("summary", "") for s in sources)
