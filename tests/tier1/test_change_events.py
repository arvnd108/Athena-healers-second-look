...
class TestDiff:
    ...
    def test_per_property_provenance_is_ignored_too(self):
        ...
        assert diff_properties(
            {"drug_class_retrieved_at": "old"}, {"drug_class_retrieved_at": "new"}
        ) == {}
...
class TestBuildEvent:
    def test_no_change_returns_none_rather_than_an_empty_event(self):
        ...
        assert build_event(
            node_type="Gene", node_id="ABL1",
            old_props={"symbol": "ABL1"}, new_props={"symbol": "ABL1"},
            source="CIViC",
        ) is None

    def test_records_what_changed(self):
        event = build_event(
            node_type="EvidenceItem", node_id="238",
            old_props={"evidence_level": "C"}, new_props={"evidence_level": "B"},
            source="CIViC", retrieved_at="2026-08-23T00:00:00+00:00",
        )
    ...
    def test_to_dict_is_json_serialisable_with_an_explicit_shape(self):
        ...
        assert payload["changed"]["status"] == {
            "old": "RECRUITING", "new": "ACTIVE_NOT_RECRUITING"
        }
...
class TestSinks:
    def test_null_sink_counts_what_it_drops(self):
        ...
        event = build_event(
            node_type="Gene", node_id="X", old_props={"a": 1}, new_props={"a": 2},
            source="t",
        )
        sink.emit(event)
    ...
    def test_jsonl_sink_round_trips(self, tmp_path):
        ...
        for level in ("B", "A"):
            sink.emit(
                build_event(
                    node_type="EvidenceItem", node_id="1",
    ...
    def test_a_truncated_final_line_does_not_lose_the_whole_log(self, tmp_path):
        ...
        sink = JsonlChangeEventSink(path)
        sink.emit(
            build_event(node_type="Gene", node_id="A", old_props={"a": 1},
                        new_props={"a": 2}, source="t")
        )
    ...
    def test_collecting_sink_keeps_order(self):
        ...
        for node_id in ("1", "2"):
            sink.emit(
                build_event(node_type="Gene", node_id=node_id, old_props={"a": 1},
                            new_props={"a": 2}, source="t")
            )
